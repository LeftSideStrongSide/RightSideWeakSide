...
if (!empty($_SESSION['username']) && Input::has('item_name') && Input::has('description') && Input::has('price')){
	...
	if(empty($errors)){
		...
		$newAd->save();
		// if the edit is successful, redirect to their users page
...
<body>
	<main>
		...
		<?php foreach($errors as $error): ?>
			<p><?= $error ?></p>
		<?php endforeach; ?>
		<form method = "POST" action="ads.edit.php?id=<?=$_GET['id']?>" enctype="multipart/form-data" role="form">
